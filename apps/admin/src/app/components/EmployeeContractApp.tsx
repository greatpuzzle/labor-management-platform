...
export function EmployeeContractApp({ onClose, employeeName = "홍길동", additionalTerms = "" }: EmployeeContractAppProps) {
  ...
  const handleTTS = () => {
    toast.info("텍스트 음성 변환(TTS) 기능이 실행됩니다.", {
        description: "계약서 내용을 읽어드립니다..."
    });
  };
            ...
            <div className="w-full max-w-md flex flex-col h-screen relative bg-white">
                ...
                <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sticky top-0 z-10 shrink-0 shadow-sm">
                    ...
                    <Button 
                        ...
                        variant="ghost"
                        size="sm"
                        className="h-10 text-black hover:bg-slate-100 rounded-lg px-3 flex items-center gap-2"
                    >
                        <Volume2 className="h-5 w-5" />
                        <span className="text-sm font-semibold">읽어주기</span>
                    </Button>
                </header>
      ...
      {step === 'completed' && (
        <div className="min-h-screen bg-[rgb(255,255,255)] flex flex-col items-center justify-center p-8 text-white fixed inset-0 z-50">
            ...
            <div className="w-full max-w-sm space-y-4">
                <Button 
                    className="w-full h-16 text-xl font-bold bg-white/20 hover:bg-white/30 text-[rgb(0,0,0)] rounded-2xl shadow-lg backdrop-blur-sm border border-white/30"
                ...
                <Button 
                    className="w-full h-16 text-xl font-bold bg-white text-[#00C950] hover:bg-slate-100 rounded-2xl shadow-lg"
                    onClick={onClose}
                >
                    처음으로
                </Button>
